...
#replace the s3-bucket name: de-on-youtube-raw-us-north-1-dev
aws s3 cp . s3://de-on-youtube-raw-us-north-1-dev/youtube/raw_statistics_reference_data/ --recursive --exclude "*" --include "*.json"

# To copy all data files to its own location, following Hive-style patterns:
# This creates a separate folder raw_statisics and under that with region there will be another folder
